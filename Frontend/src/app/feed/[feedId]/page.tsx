 ...
 export default function CustomFeedPage({ params }: CustomFeedPageProps) {
     ...
     const handleAddCom = async(comId: string) => {
         const res = await axios.post(
             `http://localhost:8080/sharebox/custom-feed/add/${feedId}`,
             {
     ...
     const handleDeleteCom = async(comId: string) => {
         const res = await axios.post(
             `http://localhost:8080/sharebox/custom-feed/remove/${feedId}`,
             {
                 communityIds: [Number(comId)]
             }
         )
     ...
     useEffect(() => {
         const getFeed = async() => {
             const res = await axios.get(
                 `http://localhost:8080/sharebox/custom-feed/${feedId}`
             )
     ...
     useEffect(() => {
         if (feed && feed.ownerId != user.userId) {
             const getOwner = async() => {
                 const res = await axios.get(
                     `http://localhost:8080/sharebox/users/user/${feed?.ownerId}`
                 )
         ...
         const getPosts = async() => {
             const res = await axios.get(
                 `http://localhost:8080/sharebox/custom-feed/recent-posts/${feedId}`
             )

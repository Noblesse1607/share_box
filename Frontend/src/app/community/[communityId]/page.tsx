      ...
      <main className="w-full h-[calc(100vh-112px)] flex justify-center select-none">
        ...
        <div className="w-[80%]">
          ...
          <div className="relative flex justify-between w-full">
            ...
            <h1 className="text-textHeadingColor text-2xl ml-[180px] mt-6 font-bold">
              {community?.name}
            </h1>
            <div className="flex mt-6 mr-6 gap-4">
              {community &&
                (community?.ownerId === user.userId ||
                  community?.members.some((member: any) => member.userId === user.userId)) && (
                  <div
                    onClick={() => router.push("/createpost")}
                    className="w-[150px] h-[40px] rounded-full bg-textHeadingColor text-white flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.05] duration-150"
                  >
                    <Image src={PlusIcon} alt="Plus Icon" className="w-[15px]" />
                    <p>Create post</p>
                  </div>
                )}
              ...
              {community && community?.ownerId !== user.userId && (
                <button
                  onClick={handleJoin}
                  className="w-[80px] h-[40px] rounded-full cursor-pointer hover:scale-[1.05] duration-150 bg-textHeadingColor text-white"
                >
                  {isJoin ? "Joined" : "Join"}
                </button>
              )}
          ...
          <div className="w-full mt-16 flex justify-between">
            <div className="w-[70%] bg-red-300">test</div>
            <div className="sticky top-[100px] right-0 w-[28%] h-fit bg-boxBackground rounded-md p-4 text-textHeadingColor break-words">
              ...
              <div className="flex mt-4">
                <div className="flex items-center gap-2">
                  <Image src={EarthIcon} alt="Earth Icon" className="w-[20px]" />
                  <p className="text-sm">
                ...
                <div className="ml-4 flex items-center gap-2">
                  <Image src={MemberIcon} alt="Member Icon" className="w-[20px]" />
                  <p className="text-sm">
                    {community?.members && community?.members.length + 1} Members
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 w-full">
                ...
                <div className="mt-3 flex items-center">
                  <div className="w-[60px] h-[60px] rounded-full bg-textGrayColor1 overflow-hidden">
                    {community && <img src={owner ? owner.avatar : user.avatar} alt="Avatar" />}
                  </div>

import { useSelectedRoomContext } from "@/context/SelectedRoomContext";
 ...
 export default function ChatBox() {
     ...
     const handleClose = async() => {
         await axios.post(
             `http://localhost:8080/sharebox/chatroom/change-status?chatroomId=${selectedRoom.chatroomId}&userId=${user.userId}&status=LEAVE`
         )
     ...
     const handleSendMes = async() => {
         try {
             if (isEdit != "") {
                 ...
                 await axios.post(
                     `http://localhost:8080/sharebox/message/edit/${isEdit}`,
                     formData
                 )
                 setIsEdit("");
                 setContent("");
                 ...
                 if (fileArr.length > 0) {
                     ...
                     const uploadPromises = fileArr.map(async (file_) => {
                         ...
                         await axios.post(
                             `http://localhost:8080/sharebox/message/create-media/${selectedRoom?.chatroomId}/${user.userId}/${friend?.userId}`,
                             formData
                         );
                     });
                 ...
                 if (content !== "") {
                     ...
                     const res = await axios.post(
                         `http://localhost:8080/sharebox/message/create/${selectedRoom?.chatroomId}/${user.userId}/${friend?.userId}`,
                         formData
                     );
     ...
     useEffect(() => {
         if (selectedRoom) {
             ...
             const getMesList = async() => {
                 try {
                     const res = await axios.get(
                         `http://localhost:8080/sharebox/message/get/${selectedRoom.chatroomId}`
                     );
                     if (res.data.result) {
             ...
             const getFriendInfo = async() => {
                 const res = await axios.get(
                     `http://localhost:8080/sharebox/users/user/${fr.userId}`
                 )
                 if (res.data.result) {
                     setFriend({
                         ...fr,

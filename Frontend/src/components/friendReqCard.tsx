import axios from "axios";
 ...
 export default function FriendReqCard({ user, setRefresh, ownerId }: { user: any, setRefresh: (value: SetStateAction<number>) => void, ownerId: number }) {
     ...
     const handleAddFriend = async() => {
         await axios.post(
             `http://localhost:8080/sharebox/friend/response?requesterId=${user.userId}&receiverId=${ownerId}&status=ACCEPTED`
         )
         setStatus("ACCEPT")
         setTimeout(() => {
             setRefresh(n=>n+1);
     ...
     const handleDeclineFriend = async() => {
         await axios.post(
             `http://localhost:8080/sharebox/friend/response?requesterId=${user.userId}&receiverId=${ownerId}&status=REJECTED`
         )
         setStatus("REJECT")
         setTimeout(() => {
             setRefresh(n=>n+1);

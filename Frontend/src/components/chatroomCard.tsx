import axios from "axios";
 ...
 export default function ChatroomCard({ chatroom, user, setUnreadMes, setSelectedRoom, selectedRoom } : { chatroom: any, user: any, setUnreadMes: (value: SetStateAction<number>) => void, setSelectedRoom: (value: SetStateAction<any>) => void, selectedRoom: any}) {
     ...
     useEffect(() => {
         const getLatestMes = async() => {
             const res = await axios.get(
                 `http://localhost:8080/sharebox/message/getlatest/${chatroom.chatroomId}`
             )
             if (res.data.result) {
     ...
     const handleOpenChatBox = async() => {
         ...
         await axios.post(
             `http://localhost:8080/sharebox/chatroom/change-status?chatroomId=${chatroom.chatroomId}&userId=${user.userId}&status=IN`
         )
 
         const res = await axios.post(
             `http://localhost:8080/sharebox/message/setSeen/${chatroom.chatroomId}/${user.userId}`
         )
